...
export class VentaFormularioComponent implements OnInit {
  ...
  protected _cargando$: BehaviorSubject<boolean>;
  ...
  constructor(
    ...
    this._cargando$ = new BehaviorSubject<boolean>(true);
  ...
  protected actualizarResumen() {
    ...
    for (const item of this._detallesVenta) {
  ...
  public onClickAgregarProductos(): void {
    let dg = this.dialog.open(AgregarProductoVentaComponent, { 
      width: "37rem", 
      height: "26rem"
    });
    ...
    dg.afterClosed().subscribe(
        (productos: Producto[]) => {
          if (productos && productos.length > 0) {
            ...
            this._detallesVenta.push(...nuevosDetalles);
            this.actualizarResumen();
            this._detallesVenta$.next(this._detallesVenta);
          }
        }
      );
  }
