...
export class AgregarProductoVentaComponent implements OnInit, OnDestroy {
  ...
  public get filtro() { return this.productoForm.get("filtro"); }
  ...
  ngOnInit() {
    ...
    this._changeTipoSub = this.tipo.valueChanges.subscribe(() => { this.onChangeTipo(); });
  }
  ...
  public onChangeFamilia(): void {
    ...
    if (this.familia.value) {
      ...
      if (!isNaN(idFamilia)) {
        this.productos$ = this.prodSvc.listarProductosByFamilia(idFamilia);
  ...
  public onChangeTipo(): void {
    if (this.tipo.value) {
      ...
      const idTipo: number = Number(this.tipo.value);
      if (!isNaN(idTipo)) {
        this.productos$ = this.prodSvc.listarProductosByTipo(idTipo);
        return;
      }
    }
